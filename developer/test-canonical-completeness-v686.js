...
const audit=JSON.parse(fs.readFileSync(path.join(root,'data/canonical-completeness-audit-v6.86.json'),'utf8'));
const queue=JSON.parse(fs.readFileSync(path.join(root,'data/canonical-missing-data-queue-v6.86.json'),'utf8'));
function assert(c,m){if(!c){console.error('FAIL:',m);process.exitCode=1;}}
assert(audit.release==='v6.86.0','audit release');
assert(queue.release==='v6.86.0','queue release');
...
if(!process.exitCode) console.log('PASS canonical completeness v6.86');
